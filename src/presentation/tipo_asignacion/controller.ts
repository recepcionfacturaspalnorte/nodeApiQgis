...
import { Tipo_asignacionDto } from '../../domain/dtos/tipo_asignacion/tipo_asignacion.dto';
;


export class Tipo_asignacionController {

  //* DI
  constructor(
    public readonly service: Tipo_asignacionService,
  ...
  public create = async (req: Request, res: Response) => {
    try {
      const [error, objectDto] = Tipo_asignacionDto.create(req.body);
      if (error) return res.status(400).json(error);
  ...
  public update = async (req: Request, res: Response) => {
    try {
      const id = +req.params.id;
      const [error, objectDto] = Tipo_asignacionDto.create({ ...req.body, id });
      if (error) return res.status(400).json({ error });
