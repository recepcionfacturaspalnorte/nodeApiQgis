...
import { AreaDto } from '../../domain/dtos/area/area.dto';
;
...
export class AreaController {
  ...
  //* DI
  constructor(
    public readonly service: AreaService,
  ...
  public create = async (req: Request, res: Response) => {
    try {
      const [error, objectDto] = AreaDto.create(req.body);
      if (error) return res.status(400).json(error);
  ...
  public update = async (req: Request, res: Response) => {
    try {
      const id = +req.params.id;
      const [error, objectDto] = AreaDto.create({ ...req.body, id });
      if (error) return res.status(400).json({ error });
