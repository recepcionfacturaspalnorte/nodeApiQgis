...
import { Tipo_equipoDto } from '../../domain/dtos/tipo_equipo/tipo_equipo.dto';


export class Tipo_equipoController {

  //* DI
  constructor(
    public readonly service: Tipo_equipoService
    ,
  ...
  public create = async (req: Request, res: Response) => {
    try {
      ...
      const [error, objectDto] = Tipo_equipoDto.create(req.body);
      if (error) return res.status(400).json(error);
  ...
  public update = async (req: Request, res: Response) => {
    try {
      const id = +req.params.id;
      const [error, objectDto] = Tipo_equipoDto.create({ ...req.body, id });
      if (error) return res.status(400).json({ error });
