...
export class Activo_asignacionController {

  //* DI
  constructor(
    public readonly service: Activo_asignacionService,
  ...
  public create = async (req: Request, res: Response) => {
    try {
      const [error, objectDto] = Activo_asignacionDto.create(req.body);
      if (error) return res.status(400).json(error);
  ...
  public update = async (req: Request, res: Response) => {
    try {
      ...
      const [error, objectDto] = Activo_asignacionDto.create({ ...req.body, id });

      if (error) {
        console.log("Se produjo un error");

        console.log(error, objectDto);
      }
      if (error) return res.status(400).json({ error });
