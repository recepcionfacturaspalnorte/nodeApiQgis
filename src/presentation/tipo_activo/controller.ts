...
import { Tipo_activoDto } from '../../domain/dtos/tipo_activo/tipo_activo.dto';


export class Tipo_activoController {

  //* DI
  constructor(
    public readonly service: Tipo_activoService
    ,
  ...
  public create = async (req: Request, res: Response) => {
    try {
      ...
      const [error, objectDto] = Tipo_activoDto.create(req.body);
      if (error) return res.status(400).json(error);
  ...
  public update = async (req: Request, res: Response) => {
    try {
      const id = +req.params.id;
      const [error, objectDto] = Tipo_activoDto.create({ ...req.body, id });
      if (error) return res.status(400).json({ error });
