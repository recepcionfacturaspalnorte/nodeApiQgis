...
import { MarcaDto } from '../../domain/dtos/marca/marca.dto';
;
...
export class MarcaController {
  ...
  //* DI
  constructor(
    public readonly service: MarcaService,
  ...
  public create = async (req: Request, res: Response) => {
    try {
      const [error, objectDto] = MarcaDto.create(req.body);
      if (error) return res.status(400).json(error);
  ...
  public update = async (req: Request, res: Response) => {
    try {
      const id = +req.params.id;
      const [error, objectDto] = MarcaDto.create({ ...req.body, id });
      if (error) return res.status(400).json({ error });
