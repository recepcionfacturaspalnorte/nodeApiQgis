...
export class ActivoImageController {

  //* DI
  constructor(
    public readonly service: ActivoImageService,
  ...
  public create = async (req: Request, res: Response) => {
    try {
      const [error, objectDto] = ActivoImageDto.create(req.body);
      if (error) return res.status(400).json(error);
  ...
  public update = async (req: Request, res: Response) => {
    try {
      const id = +req.params.id;
      const [error, objectDto] = ActivoImageDto.create({ ...req.body, id });
      if (error) return res.status(400).json({ error });
  ...
  public delete = async (req: Request, res: Response) => {
    ...
    try {
      ...
      //console.log(deleted);
      //const ruta = `uploads/activos/${nameImage}`
      //console.log(ruta);
