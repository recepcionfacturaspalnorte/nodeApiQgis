...
import { Categoria_activoDto } from '../../domain/dtos/categoria_activo/categoria_activo.dto';
;


export class Categoria_activoController {

  //* DI
  constructor(
    public readonly service: Categoria_activoService,
  ...
  public create = async (req: Request, res: Response) => {
    try {
      const [error, objectDto] = Categoria_activoDto.create(req.body);
      if (error) return res.status(400).json(error);
  ...
  public update = async (req: Request, res: Response) => {
    try {
      const id = +req.params.id;
      const [error, objectDto] = Categoria_activoDto.create({ ...req.body, id });
      if (error) return res.status(400).json({ error });
