...
import { Ticket_tipoDto } from '../../domain/dtos/ticket_tipo/ticket_tipo.dto';




export class Ticket_tipoController {

  //* DI
  constructor(
    public readonly service: Ticket_tipoService
    ,
  ...
  public create = async (req: Request, res: Response) => {
    try {
      ...
      const [error, objectDto] = Ticket_tipoDto.create(req.body);
      if (error) return res.status(400).json(error);
  ...
  public update = async (req: Request, res: Response) => {
    try {
      const id = +req.params.id;
      const [error, objectDto] = Ticket_tipoDto.create({ ...req.body, id });
      if (error) return res.status(400).json({ error });
