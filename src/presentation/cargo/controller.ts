...
import { CargoDto } from '../../domain/dtos/cargo/cargo.dto';
...
export class CargoController {
  ...
  //* DI
  constructor(
    public readonly service: CargoService,
  ...
  public create = async (req: Request, res: Response) => {
    try {
      const [error, objectDto] = CargoDto.create(req.body);
      if (error) return res.status(400).json(error);
  ...
  public update = async (req: Request, res: Response) => {
    try {
      const id = +req.params.id;
      const [error, objectDto] = CargoDto.create({ ...req.body, id });
      if (error) return res.status(400).json({ error });
